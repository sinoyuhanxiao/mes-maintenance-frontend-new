<template>
  ...
  >
    ...
    <div class="card-content">
      ...
      <div class="row-3" v-if="request.equipment_node_tree && request.equipment_node_tree.length > 0">
        <div class="meta-item">
...
const equipmentLabel = computed(() => {
  const tree = props.request?.equipment_node_tree
  if (!tree || tree.length === 0) return 'N/A'

  // Get the last node in the tree (most specific)
  const lastNode = tree[tree.length - 1]
  return lastNode?.name || 'Equipment'
})
