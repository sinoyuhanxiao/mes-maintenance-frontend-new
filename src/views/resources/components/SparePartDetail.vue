<template>
  <div class="sp-detail">
    ...
    <div class="scroll-area">
      ...
      <div class="progress-header">
        <div class="progress-stats">
          ...
          <div class="stat-item">
            ...
            <span class="stat-value">{{ data?.reorder_point ?? '-' }}</span>
          </div>

          <!--            <div class="stat-item">-->
          <!--              <span class="stat-label">Minimum stock level:</span>-->

          <!--              <span class="stat-value">{{ data?.minimum_stock_level ?? '-' }}</span>-->
          <!--            </div>-->

          <!--            <div class="stat-item">-->
          <!--              <span class="stat-label">Maximum stock level:</span>-->

          <!--              <span class="stat-value">{{ data?.maximum_stock_level ?? '-' }}</span>-->
          <!--            </div>-->
        </div>
      </div>
      ...
      <div>
        <div style="display: flex; flex-direction: row; justify-content: space-between; align-items: center; ">
          ...
          <div v-if="data.inventories?.length > 0">
            <el-button :icon="Coordinate" :type="'success'" plain @click="handleCreateLotTransaction">{{ 'Create Lot Transaction' }}</el-button>
          </div>
        </div>
          ...
          <el-card v-for="(lot, i) in data.inventories" :key="i" shadow="never">
            <div class="card-root-content">
              ...
              <div class="card-info">
                ...
                <div class="card-row">
                  <div class="row-value">{{ 'Number of Units' }}: {{ lot.unit_in_stock ?? '-' }}</div>
        ...
        <!-- Material Lot Transfer Logs -->
        <div class="lot-transaction-table">
          <el-table :empty-text="'No Transaction Record'" :data="inventory_transaction_logs" border size="small">
            <el-table-column :label="'Transaction ID'" sortable prop="id" width="150" />

            <el-table-column :label="'Transaction Type'" sortable prop="transaction_type.name" width="160" />

            <el-table-column :label="'Quantity'" sortable prop="quantity_change" width="120" />

            <el-table-column :label="'From Lot'" prop="from_inventory.id">
              <template #default="{ row }">
                {{ row.from_inventory?.id ?? '-'}}
              </template>
            </el-table-column>

            <el-table-column :label="'To Lot'" prop="to_inventory.id">
              <template #default="{ row }">
                {{ row.to_inventory?.id ?? '-'}}
              </template>
            </el-table-column>

            <el-table-column :label="'Created At'" sortable prop="created_at" width="200">
              <template #default="{ row }">
      ...
      <!-- Material Suppliers -->
      <div>
        <h3 class="section-title">Material Supplier</h3>
        ...
        <el-empty v-else :description="'No Material Supplier'" :image-size="60" />
      </div>
...
//   {
//     created_at : '2024-08-28T00:18:25.272614Z',
//     created_by : 47,
//     updated_at : '2024-08-29T20:51:50.095287Z',
//     updated_by : 47,
//     status : 1,
//     id : 208,
//     location_id : 589,
//     unit_in_stock : 127,
//     material_type_id : 3,
//     batch_number : 'TEST BATCH NUMBER',
...
function handleCreateLotTransaction() {
  ElMessage( {
    message : 'This feature is still work in progress.',
    type : 'warning',
    plain : true
  } )
}
