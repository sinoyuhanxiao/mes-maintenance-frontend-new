    ...
    <el-card class="panel-card">
      ...
      <div class="panel-content">
        ...
        <div v-else class="reorder-steps-mode">
          ...
          <div
            v-if="selectedStepIds.length > 0 && uiHints.showBulkToolbarOnSelect"
            class="bulk-actions-toolbar"
          >
            <div class="bulk-actions-info">
              {{ selectedStepIds.length }} step(s) selected
            </div>
...
import {
  Plus, Rank, Search, Edit, CircleCheck, Document, DataAnalysis, Paperclip,
  DocumentCopy, Star, StarFilled, Setting, Delete
...
const props = defineProps({
  ...
  // UI configuration from JSON schema
  uiHints: {
    type: Object,
    default: () => ({
      showCheckboxes: true,
...
const allSelectedRequired = computed(() => {
  ...
  const selectedSteps = props.steps.filter(step =>
    selectedStepIds.value.includes(step.id)
  )
...
const draggableKey = computed(() => {
  ...
  // This fixes the drag-and-drop issue while keeping reactivity for content changes.
  const sortedSteps = [...stepsForDrag.value].sort((a, b) => (a.id > b.id ? 1 : -1));
  return sortedSteps.map(s => `${s.id}-${s.title}-${s.type}`).join(',');
})
...
const handleVueDragStart = (evt) => {
  console.log('FloatingPanel drag started:', evt)
}

const handleVueDragEnd = (evt) => {
  console.log('FloatingPanel drag ended:', evt)
...
const handleVueDragChange = (evt) => {
  console.log('FloatingPanel drag change:', evt)
  // VueDraggable automatically updates the v-model (filteredSteps)
}
