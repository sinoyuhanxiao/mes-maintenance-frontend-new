    ...
    <div class="dialog-content">
      ...
      <div v-else class="changes-summary">
        <div class="summary-header">
          <p>Review the following modifications before saving:</p>
        </div>

        <!-- Metadata Changes -->
        <div v-if="changes.metadata.length > 0" class="change-section">
          <div class="section-header">
            <el-icon class="section-icon" style="color: #409eff"><Edit /></el-icon>
            <h4>Template Information</h4>
            <el-tag size="default" type="info" effect="plain">{{ changes.metadata.length }} change(s)</el-tag>
          </div>
        ...
        <div v-if="changes.steps_added.length > 0" class="change-section">
          <div class="section-header">
            <el-icon class="section-icon" style="color: #67c23a"><Plus /></el-icon>
            <h4>Steps Added</h4>
            <el-tag size="default" type="success" effect="plain">{{ changes.steps_added.length }} step(s)</el-tag>
          </div>
          <div class="changes-list">
            <div v-for="step in changes.steps_added" :key="step.step_id" class="change-item step-added">
              <div class="step-info">
                <div class="step-type-indicator" :style="getStepTypeStyle(step.type)">
                  <el-icon>
                    <component :is="getStepTypeIconComponent(step.type)" />
                  </el-icon>
                </div>
        ...
        <div v-if="changes.steps_modified.length > 0" class="change-section">
          <div class="section-header">
            <el-icon class="section-icon" style="color: #e6a23c"><Edit /></el-icon>
            <h4>Steps Modified</h4>
            <el-tag size="default" type="warning" effect="plain">{{ changes.steps_modified.length }} step(s)</el-tag>
          </div>
          <div class="changes-list">
        ...
        <div v-if="changes.steps_deleted.length > 0" class="change-section">
          <div class="section-header">
            <el-icon class="section-icon" style="color: #f56c6c"><Delete /></el-icon>
            <h4>Steps Deleted</h4>
            <el-tag size="default" type="danger" effect="plain">{{ changes.steps_deleted.length }} step(s)</el-tag>
          </div>
          <div class="changes-list">
            <div v-for="step in changes.steps_deleted" :key="step.step_id" class="change-item step-deleted">
              <div class="step-info">
                <div class="step-type-indicator deleted" :style="getStepTypeStyle(step.type)">
                  <el-icon>
                    <component :is="getStepTypeIconComponent(step.type)" />
                  </el-icon>
                </div>
        ...
        <div v-if="changes.steps_reordered.length > 0" class="change-section">
          <div class="section-header">
            <el-icon class="section-icon" style="color: #909399"><Sort /></el-icon>
            <h4>Steps Reordered</h4>
            <el-tag size="default" type="info" effect="plain">{{ changes.steps_reordered.length }} step(s)</el-tag>
          </div>
          <div class="changes-list">
            <div v-for="step in changes.steps_reordered" :key="step.step_id" class="change-item step-reordered">
              <div class="step-info">
                <div class="step-type-indicator" :style="getStepTypeStyle(getStepType(step.step_id))">
                  <el-icon>
                    <component :is="getStepTypeIconComponent(getStepType(step.step_id))" />
                  </el-icon>
                </div>
                <div class="step-details">
                  <div class="step-name">{{ step.label }}</div>
...
.step-type-indicator .el-icon {
  font-size: 16px;
}

.step-type-indicator.deleted {
  opacity: 0.5;
  position: relative;
}

.step-type-indicator.deleted::after {
  content: '';
  position: absolute;
  top: 50%;
  left: 2px;
