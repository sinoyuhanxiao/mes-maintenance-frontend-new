...
const stepTypeColors = computed( () => {
  const colors = {
    ...
    text : '#909399',
    attachments : '#849aec'
    // Note: service step type is currently disabled in the backend
    // service: '#df869d',
  }
  return colors
} )
...
const handleAddStep = stepType => {
  ...
  // Scroll to the newly added step after DOM update
  ...
  if ( isEditing.value ) {
    try {
      ...
      if ( settingsStore.tagsView && template.reference_id ) {
        ...
        console.log( `[Designer] Updated tag title to: ${newTitle}` )
      }
...
onMounted( async() => {
  ...
  if ( settingsStore.tagsView && hasCache() ) {
    console.log( '[Designer] Cached state detected, skipping initialization in onMounted' )
    return
  }

  // Initialize template normally if no cached state
  console.log( '[Designer] No cached state, initializing template normally' )
...
onActivated( async() => {
  ...
  if ( settingsStore.tagsView && ( hasCachedStateByKey( stableCacheKey.value ) || hasCache() ) ) {
    console.log( '[Designer] Has cached state, attempting to restore...' )
    const cachedState = restoreStateByKey( stableCacheKey.value ) || restoreCache()
    if ( cachedState ) {
      console.log( '[Designer] Restoring cached state:', cachedState )
      ...
      console.log( '[Designer] Successfully restored cached state for route:', route.path )
  ...
  if ( settingsStore.tagsView && isEditing.value && route.params.id ) {
    ...
    if ( restored ) {
      console.log( '[Designer] Restored state by templateId and migrated key:', route.params.id )
  ...
  if ( isFormEmpty ) {
    console.log( '[Designer] No cached state and form is empty, initializing template' )
    await initializeTemplate()
    renderReady.value = true
  }
} )
...
onDeactivated( () => {
  console.log( '[Designer] onDeactivated called for route:', route.path, 'tagViews:', settingsStore.tagsView )
...
watch(
  ...
  ( newId, oldId ) => {
    ...
    if ( settingsStore.tagsView && !isComponentActive.value ) {
      console.log( '[Designer] Ignoring route id change while deactivated:', oldId, '->', newId )
      return
    }
