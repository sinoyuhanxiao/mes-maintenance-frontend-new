...
// Computed
const displayItems = computed( () => ( Array.isArray( equipmentNodes.value ) ? equipmentNodes.value : [] ) )

const defaultPins = computed( () =>
  ( Array.isArray( equipmentNodes.value ) ? equipmentNodes.value : [] ).map( ( item, index ) => ( {
...
// function pickDiagramIdFromNode( n ) {
//   if ( !n || typeof n !== 'object' ) return null
//   if ( n.diagramId ) return n.diagramId
//   if ( n.diagram_id ) return n.diagram_id
//   if ( n.equipmentDiagramId ) return n.equipmentDiagramId
//   if ( n.equipment_diagram_id ) return n.equipment_diagram_id
//   const d = n.diagram || n.equipment_diagram
//   if ( d && typeof d === 'object' ) return d.id || d._id || null
//   if ( Array.isArray( n.diagram ) && n.diagram.length ) {
//     const first = n.diagram[0]
//     return first?._id || first?.id || null
//   }
//   return null
// }

// const resolvedDiagramId = computed( () => {
//   if ( props.diagramId != null ) return String( props.diagramId )
//   const items = Array.isArray( equipmentNodes.value ) ? equipmentNodes.value : []
//   for ( const n of items ) {
//     const id = pickDiagramIdFromNode( n )
      ...
      try {
        const payload = {
          pins : validPins.map( pin => {
            ...
            const { equipment_node_id, ...pinWithoutId } = pin
            return pinWithoutId
          } )
        }

        const res = await createEquipmentDiagram( payload, props.entityId )
...
const loadPinsFromBackend = async() => {
  ...
  try {
    const diagId = diagramId.value
    if ( !diagId ) {
      return loadPinPositionsFromStorage()
    }
    const res = await getEquipmentDiagram( diagId )
    ...
    if ( pinsPayload.length ) {
      pins.value = pinsPayload.map( ( p, idx ) => ( {
        id : p.equipment_node_id,
        number : p.sequence_order ?? idx + 1,
        text : p.name,
